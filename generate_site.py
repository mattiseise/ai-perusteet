...
<!-- Google Analytics 4 (G-4ZLJF4THGV): latautuu vain suostumuksella (basic consent mode) -->
<script>
...
function trackEvent(n,p){{if(window.__gaLoaded&&typeof gtag==='function')gtag('event',n,p||{{}});}}
...
document.addEventListener('DOMContentLoaded',function(){{var c=null;try{{c=localStorage.getItem('bcai-consent')}}catch(e){{}}
if(c==='granted'){{loadGA();}}else if(c!=='denied'){{var b=document.getElementById('consent-banner');if(b)b.style.display='flex';}}}});
</script>
<style>
#consent-banner{{display:none;position:fixed;left:16px;right:16px;bottom:16px;z-index:9999;margin:0 auto;max-width:680px;background:#FFFFFF;border:1px solid #D8DEEC;border-radius:14px;box-shadow:0 8px 30px rgba(27,35,54,.18);padding:14px 18px;gap:14px;align-items:center;justify-content:space-between;flex-wrap:wrap;font-family:'Hanken Grotesk',system-ui,sans-serif;font-size:14px;color:#3A4253}}
...
function toggleDone(id){{const a=getDone(),i=a.indexOf(id);if(i>=0)a.splice(i,1);else{{a.push(id);trackEvent('lesson_complete',{{lesson_id:id,lesson_num:ALLIDS.indexOf(id)+1}});}}setDone(a);updProg();updCards();updDoneBtn(id);}}
...
<div id="consent-banner" role="dialog" aria-live="polite">
  <span>Sivusto käyttää Google Analyticsia kävijätilastointiin. Analytiikkaevästeet otetaan käyttöön vain hyväksynnälläsi.</span>
