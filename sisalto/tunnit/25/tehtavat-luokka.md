# Opiskelutehtävät: Ihminen portinvartijana — ihmisen osallistuminen päätöksentekoon

> Tämä tunti rakentaa lopputyösi viimeistä osaa. Tuotos: **Agentti: Ihminen** (5/5).

*Kaikkia ei tarvitse tehdä. Valitse tehtävistä 1. Suosittelen kuitenkin Agentti: Ihminen -tehtävää.*

> **Vinkki:** Ihmisen rooli voi olla pieni tai iso. Yksinkertaisessa FAQ-botissa ihmistä ei välttämättä tarvita lainkaan; kriittisiä päätöksiä tekevässä agentissa hyväksyntäportteja on useampi.

---

## Agentti: Ihminen — suositeltu

**Miksi tämä on tärkeää:** Agentin viimeistelytunnilla lisäät hyväksyntäsolmuja n8n:ssä. Ihmisen osallistuminen päätöksentekoon vaatii tasapainoa: liian monta hyväksyntäporttia tekee agentista hitaan, liian vähän tekee siitä riskialttiin.

### Tehtävä

Avaa muistiinpanoistasi aiemmat Agentti-pohjapiirroksesi. Kirjoita 150–200 sanaa, jaettuna kolmeen osaan:

**1. Missä kohdissa ihmisen täytyy hyväksyä.**

*Käsite:* hyväksyntäportti = kohta työnkulussa, jossa agentti pysähtyy odottamaan ihmisen päätöstä.

*Kolme yleistä sääntöä, milloin ihmistä tarvitaan:*

- **Raha tai sitoutuminen:** päätös vaikuttaa rahaan, asiakassuhteeseen tai sopimukseen
- **Havaittava puute tai virhe:** hyväksyttyä lähdettä ei löydy, lähteet ovat ristiriidassa, pakollinen tieto puuttuu, validointi epäonnistuu tai työkalu palauttaa virheen
- **Poikkeama:** tapahtuu jotain epätavallista

*Esimerkki:* "Ihmisen täytyy hyväksyä, kun agentti aikoo antaa yli 10 % alennuksen tai kun hyväksyttyä palautusehtoa ei löydy tietopohjasta."

*Sinun vuorosi:* Sääntö vai tekoäly -tunnista–3 kohtaa agenttisi työnkulussa. Voit kirjoittaa myös "ihmistä ei tarvita" — perustele silloin miksi.

**2. Millainen hyväksyntäportti on.**

*Käsite:* hyvä hyväksyntäportti antaa ihmiselle kaiken tarvittavan tiedon kerralla. Päätös pitää voida tehdä 30 sekunnissa.

*Esimerkki:* "Asiakas X (10 v. asiakkuus) pyytää 25 % alennusta. Agentti ehdottaa hyväksyntää. Hyväksy / Hylkää."

*Sinun vuorosi:* Kuvaa, miten oman agenttisi hyväksyntäportti näyttää: kanava (tiimikanava tai sähköposti), tieto, vastaustapa.

**3. Mitä tapahtuu, jos ihminen ei vastaa.**

*Käsite:* turvallinen varasuunnitelma jokaiselle hyväksyntäportille. Strategiat ovat **keskeytys tai oletushylkäys**, **eskalointi** toiselle henkilölle tai asian palauttaminen myöhemmin käsiteltäväksi. Kriittistä toimintoa ei hyväksytä hiljaisuuden perusteella.

*Esimerkki:* "Jos päällikkö ei vastaa 4 h:ssa, asia eskaloituu varapäällikölle."

*Sinun vuorosi:* Valitse jokaiselle hyväksyntäportille varasuunnitelma.

> **Sivumaininta: palautteesta kehittäminen.** Edistyneissä järjestelmissä hyväksynnät ja hylkäykset voidaan tallentaa päätöslokiin. Ihminen arvioi aineiston ja päättää, muutetaanko ohjeita, agentin ohjauskehyksen sääntöjä tai testejä. Lopputyössä riittää, että kuvaat tämän hallitun menettelyn.

### Tekoälyvaihe — testaa hyväksyntäporttisi

```
Olen suunnitellut agentilleni seuraavan hyväksyntäportin: [liitä].
Aseta itsesi sen henkilön asemaan, joka saa tämän viestin keskellä
työpäivää. Pystytkö tekemään päätöksen 30 sekunnissa? Mitä tietoa
puuttuu? Ovatko jotkin tiedot ristiriidassa?
```

> **Vinkki muistiinpanoihin:** Tämä on 5/5 — kaikki pohjat ovat nyt koossa.

---

## Tehtävä 25.1 — Milloin ihmistä tarvitaan? — syventävä

### Tehtävä

Kuusi agenttipäätöstä. Päätä jokaiselle, tarvitaanko ihmisen hyväksyntä, ja perustele havaittavalla ehdolla (raha tai sitoumus / puuttuva tieto tai virhe / poikkeama tai määritelty riskiluokka).

| Agentin päätös | Ihmistä tarvitaan? | Perustelu |
|---|---|---|
| Asiakas haluaa 50 % alennuksen pitkälle sopimukselle | **KYLLÄ** | Raha — iso alennus vaatii myyntipäällikön päätöksen |
| Asiakas kysyy: "Mikä on tuotteen hinta?" | **EI** | Tieto löytyy hyväksytystä lähteestä, eikä mikään määritelty eskalointiehto täyty |
| Asiakas haluaa vaihtaa tuotteen väriä (väri varastossa) | ? | |
| Uusi liikekumppani pyytää integraatiota tietokantaamme | ? | |
| Asiakas haluaa palauttaa tuotteen (palautusaika voimassa) | ? | |
| Agentti havaitsee lokeista 100 ostoa yhden tunnin sisällä | ? | |

### Sovellus omaan agenttiin

Listaa 4–5 yleisintä päätöstä omalle agentillesi. Sama analyysi.

> **Vinkki muistiinpanoihin:** Jos taulukko muuttaa käsitystäsi, päivitä Agentti: Ihminen.

---

**Agentti: Ihminen valmis — kaikki 5/5 pohjapiirrosta koossa, seuraavaksi rakentaminen**
