...
<figure class="ai-demo"><span class="ai-demo__tag">// sama tieto kahdessa muodossa — tehtävä ratkaisee kumpi voittaa</span>
<div class="ai-demo__stage" style="display:flex;align-items:center;justify-content:center;height:312px">
  <div class="l06-wrap" role="img" aria-label="Sama myyntidata näytetään kaaviona ja taulukkona. Ensimmäinen tehtävä on laskea tarkka muutos: kaaviosta saa vain arvion, taulukosta tarkan arvon 44,4 prosenttia — piste taulukolle. Toinen tehtävä on huomata, että kaavion asteikko alkaa kymmenestä: sen näkee kaaviosta, ei taulukosta — piste kaaviolle. Kumpikin muoto voittaa toisen tehtävän: muoto valitaan tehtävän mukaan.">
    ...
    <div class="l06-p pk"><i class="l06-ph">KUVA · KAAVIO</i>
      ...
      <span class="l06-chip cb1">✓ akseli alkaa 10:stä</span></div>
...
<figcaption class="ai-demo__cap">Aineistomuoto ei ole koriste. Kaaviosta hahmottaa suunnan ja esitystavan valinnat, taulukko säilyttää tarkat arvot laskentaan. Sama aineisto — kumpikin muoto voittaa oman tehtävänsä. Vie hiiri kuvan päälle pysäyttääksesi.</figcaption></figure>
<style>
.l06-wrap{position:relative;width:560px;height:296px;font-family:var(--font-mono);animation:l06w 16s infinite}
.l06-wrap:hover,.l06-wrap:hover *{animation-play-state:paused}
...
.l06-task.ta{animation:l06ta 16s infinite}
.l06-task.tb{border-color:#C9B7F1;animation:l06tb 16s infinite}
...
.l06-ph{display:block;font-style:normal;font-size:11px;font-weight:700;letter-spacing:.1em;color:#C9B7F1}
.l06-ph.pht{color:#46C7CF}
.l06-ax{position:absolute;left:12px;bottom:34px;font-size:11px;color:#B9C2DA}
.l06-bar{position:absolute;bottom:36px;width:30px;background:#46C7CF;border-radius:3px 3px 0 0}
.l06-bar.b1{left:44px;height:26px;opacity:.75}
.l06-bar.b2{left:92px;height:40px}
.l06-yr{position:absolute;bottom:22px;font-size:9.5px;color:#8B94B3}
.l06-yr.y1{left:40px}.l06-yr.y2{left:88px}
.l06-row{position:absolute;left:16px;font-size:12px;line-height:1.2;color:#EAEEF8}
.l06-row.r1{top:38px}.l06-row.r2{top:60px}
.l06-chip{position:absolute;font-size:10.5px;line-height:1.2;font-weight:700;border-radius:7px;padding:3px 7px;opacity:0;white-space:nowrap}
.l06-chip.ca1{right:10px;top:34px;color:#FFD79A;border:1px solid #FFD79A;background:rgba(255,215,154,.07);animation:l06ca1 16s infinite}
.l06-chip.cb1{right:10px;bottom:10px;color:#7FD0A8;border:1px solid #7FD0A8;background:rgba(127,208,168,.08);animation:l06cb 16s infinite}
.l06-chip.ca2{right:10px;top:34px;color:#7FD0A8;border:1px solid #7FD0A8;background:rgba(127,208,168,.08);animation:l06ca2 16s infinite}
.l06-chip.cb2{right:10px;bottom:10px;color:#FFD79A;border:1px solid #FFD79A;background:rgba(255,215,154,.07);animation:l06cb 16s infinite}
.l06-log{position:absolute;left:0;width:560px;text-align:center;font-size:11.5px;line-height:1.2;color:#7FD0A8;opacity:0}
.l06-log.lg1{top:204px;animation:l06lg1 16s infinite}
.l06-log.lg2{top:226px;animation:l06lg2 16s infinite}
.l06-syn{position:absolute;left:0;top:256px;width:560px;text-align:center;font-size:12px;font-weight:600;color:#FFD79A;opacity:0;animation:l06syn 16s infinite}
@keyframes l06w{0%{opacity:0}4.4%{opacity:1}96.5%{opacity:1}100%{opacity:0}}
...
@keyframes l06ca1{0%,12%{opacity:0}16%{opacity:1;transform:rotate(0)}18%{transform:rotate(-2.5deg)}20%{transform:rotate(2.5deg)}22%{transform:rotate(0)}45%{opacity:1}49%,100%{opacity:.4}}
@keyframes l06ca2{0%,18%{opacity:0}22%{opacity:1}45%{opacity:1}49%,100%{opacity:.4}}
...
@media (prefers-reduced-motion:reduce){
  ...
  .l06-wrap,.l06-task.tb,.l06-chip.cb1,.l06-chip.cb2,.l06-log,.l06-syn{opacity:1}
  .l06-task.ta{opacity:0}
  .l06-chip.ca1,.l06-chip.ca2{opacity:.4}
}
</style>
