...
Kaikki, mitä olet oppinut — muisti, konteksti, työkalut, suunnittelumallit, turvallisuus — johtaa yhdelle tärkeälle ajatukselle: **agentti ei voi tehdä kaikesta yksin**. Joissa tilanteissa agentti täytyy pysäyttää ja odottaa, kunnes ihminen sanoo "kyllä, jatka" tai "ei, älä tee sitä".
...
Näillä säännöillä voit jakaa pöytä. Neljäsosaaa päätöksistä agentti tekee yksin (rutiinit), ja neljäsosa täytyy ihmisen hyväksyntä (kritiikat). Loput puoli jakautuu — jotkut vaativat hyväksyntää varmautumisvaiheessa, toiset vasta jälkikäteen, kun ihminen tarkistaa lokeja.
...
Kun tiedät, mitkä päätökset vaativat hyväksyntä, seuraava askel on **rakentaa hyväksyntäportit**. Portti on pisteen järjestelmässä, jossa agentti pysähtyy ja odottaa ihmisen vastausta.
...
- **Oletusarvo**: Jos no vastaus, hyväksyntä katsotaan annetuksi (vaatii muussaksi kriittistä — esim. asiakkaalle varattu tuote myydään muille).
...
Portti näyttää: Tietoturvaanalyysi, integratiiviset riskit, liiketoiminnan hyödyt.
...
**2. Agentti-analyysi**: Agentti lukee viestin, etsii asiakkaan tilauksen historiasta ja analyyaa. Löytää: "Tilaus tehty 5 päivää sitten. Palautusaika on 14 päivää. Asiakas on oikeutettu."

**3. Hyväksyntäportti 1**: "Oletko varma, että palautusaika on voimassa?" Agentti näyttää perustele. Ihminen vahvistaa yhdessä sekunnissa. "Kyllä, vahvistettu."

**4. Agentti-toiminta**: Agentti lähettää asiakkaalle palautuksesta tarvittavat tiedot — palautuslomake, kuljetus­ohjeet, takuun ehdot.
...
## Kohti n8n-projekteja

Kaikki, mitä tässä oppitunnissa olet oppinut — hyväksyntäportit, human-in-the-loop-työnkulut, agentin oppiminen — tulevat elämään käytännössä seuraavissa oppitunneissa. N8n on **visuaalinen automaaatio-alusta**, jossa näet konkreettisesti, kuinka:

- Rakennit **hyväksyntäportit** ja liität ne agentin päätöksiin
- Suunnittelet **työnkulkuja**, joissa agentti ja ihminen tekevät yhteistyötä
- Tallenet **palautteen** joka hyväksynnästä
- Loggit näyttävät, **mitä tapahtui ja miksi**
- Voit **kumota tai korjata** virheitä nopeasti

N8n:ssä näet visuaalisesti:
- Pistettä, joissa työnkulku pysähtyy ja odottaa ihmisen hyväksyntä
- Ehdot ("if X, then Y")
- Looppeja ("tee tämä kaikille asiakkaille")
- Virheenkäsittely ("jos API epäonnistuu, mitä tehdään seuraavaksi")

Kun opiskelet n8n:ää seuraavissa oppitunneissa, muista: **jokainen lohko, jonka vedät ruudulle, on vaihe agentin ajattelussa**. Hyväksyntälohko on piste, jossa agentti pysähtyy ja odottaa ihmisen sanaa. API-haku on vaihe, jossa agentti noutaa tietoa. Ehdolohko on vaihe, jossa agentti tekee päätöksen. Yhdessä nämä lohkot muodostavat toimivan agentin.
